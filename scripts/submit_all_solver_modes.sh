...
# qsub -N OM2-1_AA_SM -v NONLINEAR_SOLVER=anderson,AA_SOLVER=SpeedMapping${EXTRA_VARS},INITIAL_AGE=TMage scripts/ACCESS-OM2-1_GPU_job.sh
# count=$((count + 1))
# echo "[$count] Submitted anderson/SpeedMapping. Waiting ${DELAY}s..."
# sleep $DELAY

# # Anderson — NLsolve
# qsub -N OM2-1_AA_NLs -v NONLINEAR_SOLVER=anderson,AA_SOLVER=NLsolve${EXTRA_VARS},INITIAL_AGE=TMage scripts/ACCESS-OM2-1_GPU_job.sh
# count=$((count + 1))
# echo "[$count] Submitted anderson/NLsolve. Waiting ${DELAY}s..."
# sleep $DELAY

# # Anderson — SIAMFANL
# qsub -N OM2-1_AA_SIA -v NONLINEAR_SOLVER=anderson,AA_SOLVER=SIAMFANL${EXTRA_VARS},INITIAL_AGE=TMage scripts/ACCESS-OM2-1_GPU_job.sh
# count=$((count + 1))
# echo "[$count] Submitted anderson/SIAMFANL. Waiting ${DELAY}s..."
# sleep $DELAY

# # Anderson — FixedPoint
# qsub -N OM2-1_AA_FP -v NONLINEAR_SOLVER=anderson,AA_SOLVER=FixedPoint${EXTRA_VARS,INITIAL_AGE=TMage} scripts/ACCESS-OM2-1_GPU_job.sh
# count=$((count + 1))
...
# qsub -N OM2-1_AA_Pi -v NONLINEAR_SOLVER=anderson,AA_SOLVER=Picard${EXTRA_VARS:+,$EXTRA_VARS} scripts/ACCESS-OM2-1_GPU_job.sh
# count=$((count + 1))
