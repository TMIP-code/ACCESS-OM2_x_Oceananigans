...
export PARENT_MODEL EXPERIMENT
...
log_dir=logs/python/$PARENT_MODEL/$EXPERIMENT/AABW
mkdir -p "$log_dir"
...
echo "Running compute_AABW_depthspace.py"
python3 src/compute_AABW_depthspace.py "$PARENT_MODEL" "$EXPERIMENT" \
    &> "$log_dir/compute_AABW_depthspace_${job_id}.log"
echo "Done computing depth-space AABW for $PARENT_MODEL/$EXPERIMENT"
echo "Logged output in $log_dir/compute_AABW_depthspace_${job_id}.log"
