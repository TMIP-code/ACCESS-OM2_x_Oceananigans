...
export VELOCITY_SOURCE W_FORMULATION TIMESTEPPER ENABLE_AGE_SOLVE
...
if [[ "$SUBMIT_MATRIX" == "true" ]]; then
    echo "Submitting matrix build job (VELOCITY_SOURCE=$VELOCITY_SOURCE, TIMESTEPPER=$TIMESTEPPER, ENABLE_AGE_SOLVE=$ENABLE_AGE_SOLVE)"
    qsub -v PARENT_MODEL="$PARENT_MODEL",VELOCITY_SOURCE="$VELOCITY_SOURCE",TIMESTEPPER="$TIMESTEPPER",ENABLE_AGE_SOLVE="$ENABLE_AGE_SOLVE" \
