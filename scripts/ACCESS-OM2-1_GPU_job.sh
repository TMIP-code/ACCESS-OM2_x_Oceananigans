...
if [ "$TRACE_SOLVER_HISTORY" = "yes" ] && { [ "$NONLINEAR_SOLVER" = "newton" ] || [ "$NONLINEAR_SOLVER" = "anderson" ]; }; then
    ...
    qsub -v VELOCITY_SOURCE="$VELOCITY_SOURCE",W_FORMULATION="$W_FORMULATION",ADVECTION_SCHEME="$ADVECTION_SCHEME",TIMESTEPPER="$TIMESTEPPER",NONLINEAR_SOLVER="$NONLINEAR_SOLVER" \
