...
for velocity_source in "${velocity_sources[@]}"; do
  ...
  for w_formulation in "${w_formulations[@]}"; do
    ...
    for advection_scheme in "${advection_schemes[@]}"; do
      ...
      qsub -v PARENT_MODEL=ACCESS-OM2-1,VELOCITY_SOURCE="${velocity_source}",W_FORMULATION="${w_formulation}",ADVECTION_SCHEME="${advection_scheme}",ENABLE_AGE_SOLVE=true scripts/ACCESS-OM2-1_matrix_job.sh
      count=$((count + 1))
    done
  done
done
