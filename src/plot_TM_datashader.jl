"""
Datashader diagnostic plots for transport matrices.

Compares the nonzero values of the averaged transport matrices (avg12a, avg12b,
avg24) against the constant (time-mean) matrix using datashader density plots
on signed-log (pseudolog10) axes.

See also: `plot_TM_scatter.jl` for simple scatter plots.

Environment variables: PARENT_MODEL, VELOCITY_SOURCE, W_FORMULATION,
  ADVECTION_SCHEME, TIMESTEPPER
"""

@info "Loading packages for TM datashader plots"
...
@info "TM datashader plot configuration"
...
for label in avg_labels
    ...
    outfile = joinpath(plots_dir, "nzval_datashader_$(label)_vs_const.png")
    save(outfile, ds_fig)
    @info "Saved $outfile"
...
flush(stdout); flush(stderr)
@info "TM datashader plots complete"
