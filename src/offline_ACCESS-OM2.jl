...
@inline linear_source_sink(i, j, k, grid, clock, fields, params) = ifelse(k ≥ grid.Nz, -fields.autodifftracer[i, j, k] / params.relaxation_timescale, 0.0)
...
linear_forcing = (
    autodifftracer = linear_dynamics,
)
...
J = let
    ...
    function mytendency(autodifftracer1D, clock)
        ...
        set!(jacobian_model, autodifftracer = autodifftracer3D)
        autodifftracer_tendency = CenterField(grid, Real)

        autodifftracer_advection = jacobian_model.advection[:autodifftracer]
        autodifftracer_forcing = jacobian_model.forcing[:autodifftracer]
        autodifftracer_immersed_bc = immersed_boundary_condition(jacobian_model.tracers[:autodifftracer])
    ...
    prep = prepare_jacobian(mytendency, sparse_forward_backend, autodifftracer1D, Constant(times[1]))

    @info "Compute the Jacobian"
    J = 1 / 12 * mapreduce(+, eachindex(times)) do i
        @info "month $i"
        jacobian(mytendency, prep, sparse_forward_backend, autodifftracer1D, Constant(times[i]))
    end
