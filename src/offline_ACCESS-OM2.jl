...
# W_FORMULATION = get(ENV, "W_FORMULATION", VELOCITY_SOURCE == "mass_transports" ? "prescribed" : "diagnose")
# @assert W_FORMULATION in ("diagnose", "prescribed") "W_FORMULATION must be one of: diagnose, prescribed"
# FREE_SURFACE = get(ENV, "FREE_SURFACE", "prescribed_eta")
# @assert FREE_SURFACE in ("prescribed_eta", "zero", "none") "FREE_SURFACE must be one of: prescribed_eta, zero, none"
...
# @show outputdir
# mkpath(outputdir)
# age_dir = joinpath(outputdir, "age")
# mkpath(age_dir)
# save_grid = false
...
# end
# if FREE_SURFACE == "prescribed_eta"
...
if ENABLE_PLOTTING
    ...
    #     )
    #     Colorbar(fig[1, 2], plt)
    #     save(joinpath(age_dir, "final_age_k$(k)_$(parentmodel)_$(arch_str).png"), fig)
    # end
...
Colorbar(fig[1, 2], plt)
jacobian_dir = joinpath(outputdir, "matrices", run_mode_tag)
mkpath(jacobian_dir)
save(joinpath(jacobian_dir, "$(parentmodel)_jacobian2_$(run_suffix).png"), fig)
...
hist_dir = joinpath(outputdir, "age", run_mode_tag, "histograms")
mkpath(hist_dir)
save(joinpath(hist_dir, "initial_steady_age_coarsened_histogram_$(parentmodel)_$(run_suffix).png"), fig)
...
fig, ax, plt = hist(age_init_vec)
save(joinpath(hist_dir, "initial_steady_age_full_histogram_$(parentmodel)_$(run_suffix).png"), fig)
...
@time sol! = solve(nonlinearprob!,
    ...
    @info "Skipping matrix build / nonlinear solve (ENABLE_MATRIX_BUILD = false)"
    flush(stdout)
end
